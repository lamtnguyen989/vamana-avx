...
set -euo pipefail
SHARD_DIR="${1:?usage: build_index.sh <shard_dir> <index_dir> [R] [L] [alpha] [seed]}"
INDEX_DIR="${2:?usage: build_index.sh <shard_dir> <index_dir> [R] [L] [alpha] [seed]}"
R="${3:-32}"
L="${4:-64}"
ALPHA="${5:-1.20}"
SEED="$6"
...
for f in "$SHARD_DIR"/shard_*.vecf; do
    ...
    "$BUILD_BIN" "$f" "$out" "$R" "$L" "$ALPHA" "$SEED" &
    pids+=($!)
done
